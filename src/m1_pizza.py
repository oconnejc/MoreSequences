...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(0, 15),  # All numbers are approximate.
                rg.Point(15,0),
                rg.Point(-15,0),
                rg.Point(0,-15)]
    circle = rg.Circle(rg.Point(0, 0), 15)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 5:
    title = 'PIZZA test 5:  10 slices, thin (thickness=2) black lines.'
    window = rg.RoseWindow(400, 400, title)
...
def run_test_polygon():
    ...
    title = 'POLYGON test3:  7 segments with pink lines.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 7, 'pink', 5)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    linel = rg.Line(points[0],points[number_of_segments-1])
    linel.color = color
    linel.thickness = thickness
    linel.attach_to(window)
    window.render()
    for k in range(number_of_segments-1):
        generate_points_on_circle(circle, number_of_segments)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  300 segments, hops of 61.'
